...
struct AudioDecoder {
    DecoderType type;
    void* decoder_instance;     /* Pointer to specific decoder (WaveFile*, etc.) */
...
AudioDecoder* audio_decoder_open(const char* filepath) {
    ...
    switch (type) {
        ...
        case DECODER_TYPE_FLAC:
            /* Future: FLAC decoder implementation */
            snprintf(decoder->error_msg, sizeof(decoder->error_msg),
                     "FLAC decoder not yet implemented");
            free(decoder);
            return NULL;
...
size_t audio_decoder_read_samples(AudioDecoder* decoder, void* buffer, size_t num_samples) {
    ...
    switch (decoder->type) {
        ...
        case DECODER_TYPE_FLAC:
            /* Future: FLAC read implementation */
            return -1;
...
int audio_decoder_seek(AudioDecoder* decoder, uint32_t sample_position) {
    ...
    switch (decoder->type) {
        ...
        case DECODER_TYPE_FLAC:
            /* Future: FLAC seek implementation */
            return -1;
...
uint32_t audio_decoder_tell(AudioDecoder* decoder) {
    ...
    switch (decoder->type) {
        ...
        case DECODER_TYPE_FLAC:
            /* Future: FLAC tell implementation */
            return 0;
